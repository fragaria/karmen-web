...
const FaqItem = ({ resources, site, location, ...props }) => {
  ...
  return (
    <article
      className={"v-faq-resources__item " + (active ? 'active' : '')}
    >
      <div className="v-faq-resources__item__header" onClick={toggleItem}>
        <h2 className="v-faq-resources__item__headline">{frontmatter.title}</h2>
        <div className={"v-faq-resources__item__button " + (active ? 'active' : '')}></div>
      </div>
      <div
        ...
        <div className="v-faq-resources__item__content__inner"
